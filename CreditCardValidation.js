...
const validateCred = (arr) => {
  ...
  let sum = arr[arrSize - 1];
  const parity = arrSize % 2;
  for(let counter = 0; counter < arrSize - 1; counter++){
    let digit = arr[counter];
    if( counter % 2 === parity){
      digit = arr[counter] * 2;
      if (digit > 9){
        digit -= 9;
...
const findInvalidCards = (batchArr) => {
  const badCards = batchArr.filter(card => !(validateCred(card)));
  return badCards;
}

const idInvalidCardCompanies = (badCards) => {
  let invalidCardCompanies = [];
  for (badCard of badCards){
    switch (badCard[0]){
        case 3:
